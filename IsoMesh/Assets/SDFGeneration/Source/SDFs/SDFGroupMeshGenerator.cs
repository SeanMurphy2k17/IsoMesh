...
namespace IsoMesh
{
    ...
    public class SDFGroupMeshGenerator : MonoBehaviour, ISDFGroupComponent 
    {
        ...
        // SDF data buffers (stored from UpdateDataBuffer for use in UV generation)
        private ComputeBuffer m_sdfDataBuffer;
        private ComputeBuffer m_sdfMaterialBuffer;
        private int m_sdfDataCount;
        ...
        private UVGenerationResult GenerateUVsComputeShader(NativeArray<Vector3> vertices, NativeArray<Vector3> normals, NativeArray<int> triangles, int vertexCount, int triangleCount)
        {
            ...
            // Pass material data for packed UV support
            UVComputeShader.SetBuffer(kernel, "_MeshVertexMaterials", m_meshVertexMaterialsBuffer);
            
            // Pass SDF data buffers needed for MapUV function
            if (m_sdfDataBuffer != null && m_sdfDataBuffer.IsValid())
            {
                UVComputeShader.SetBuffer(kernel, "_SDFData", m_sdfDataBuffer);
                UVComputeShader.SetBuffer(kernel, "_SDFMaterials", m_sdfMaterialBuffer);
                UVComputeShader.SetInt("_SDFDataCount", m_sdfDataCount);
            }
...
uvResult.expandedUVs != null && uvResult.remappedTriangles != null)
            ...
            // Only assign collision mesh if it has valid triangles
            if (MeshCollider && m_mesh.triangles.Length > 0 && m_mesh.vertexCount >= 3)
            {
                try
                {
                ...
                {
                    Debug.LogWarning($"Failed to assign collision mesh: {e.Message}. Disabling MeshCollider.");
                    MeshCollider.enabled = false;
                }
            }
            else if (MeshCollider)
            {
                // Disable collider for invalid meshes
                MeshCollider.enabled = false;
            }
        }
        ...
        private void CreateVariableBuffers()
        {
            ...
            // Safety cap for all mesh buffers to prevent 2GB limit issues
            int maxSafeBufferSize = 20000000; // ~20M elements
            int actualBufferSize = countCubed * 3;
            int safeBufferSize = Mathf.Min(actualBufferSize, maxSafeBufferSize);
            
            if (actualBufferSize > maxSafeBufferSize)
            {
                Debug.LogError($"Voxel count too high! Requested: {actualBufferSize} elements, capped to: {safeBufferSize}. " +
                              $"Reduce Cell Count in Voxel Settings. Current cubed count: {countCubed}");
            }

            m_meshVerticesBuffer = new ComputeBuffer(safeBufferSize, sizeof(float) * 3, ComputeBufferType.Structured);
            m_meshNormalsBuffer = new ComputeBuffer(safeBufferSize, sizeof(float) * 3, ComputeBufferType.Structured);
            m_meshTrianglesBuffer = new ComputeBuffer(safeBufferSize, sizeof(int), ComputeBufferType.Structured);
            //m_meshUVsBuffer = new ComputeBuffer(safeBufferSize, sizeof(float) * 2, ComputeBufferType.Structured); // PACKED INTO VERTEX COLORS
            m_meshVertexColoursBuffer = new ComputeBuffer(safeBufferSize, sizeof(float) * 4, ComputeBufferType.Structured);
            m_meshVertexMaterialsBuffer = new ComputeBuffer(safeBufferSize, SDFMaterialGPU.Stride, ComputeBufferType.Structured);

            m_intermediateVertexBuffer = new ComputeBuffer(safeBufferSize, NewVertexData.Stride, ComputeBufferType.Append);
        ...
        public void UpdateDataBuffer(ComputeBuffer computeBuffer, ComputeBuffer materialBuffer, int count)
        {
            ...
            // Store buffers for use in UV generation compute shader
            m_sdfDataBuffer = computeBuffer;
            m_sdfMaterialBuffer = materialBuffer;
            m_sdfDataCount = count;
