...
// Original SDF function without noise (kept for compatibility)
...
// Simple noise functions for surface detail
float noiseHash(float n) { return frac(sin(n) * 43758.5453123); }
...
float noise3D(float3 p) {
    ...
    return lerp(lerp(lerp(noiseHash(n + 0.0), noiseHash(n + 1.0), f.x),
                     lerp(noiseHash(n + 57.0), noiseHash(n + 58.0), f.x), f.y),
                lerp(lerp(noiseHash(n + 113.0), noiseHash(n + 114.0), f.x),
                     lerp(noiseHash(n + 170.0), noiseHash(n + 171.0), f.x), f.y), f.z);
}

float fractalNoise3D(float3 p, float seed, int octaves, float frequency) {
    float result = 0.0;
    float amplitude = 1.0;
    p += seed * 0.1; // Apply seed offset
    for (int i = 0; i < octaves; i++) {
        result += noise3D(p * frequency) * amplitude;
        frequency *= 2.0;
        amplitude *= 0.5;
    }
    return result;
...
// Enhanced SDF function with Perlin noise surface detail
float sdf_with_noise(float3 p, SDFGPUData data, SDFMaterialGPU material)
{
    // Get base SDF distance
    float baseDist = sdf(p, data);
...
float Map(float3 p)
{
    ...
    for (int i = 0; i < _SDFDataCount; i++)
    {
        ...
        {
            // Get material data for noise displacement
...
SDFMaterialGPU MapColour(float3 p)
{
    ...
    final.UsePackedUVs = 0;
    
    // Initialize noise fields
    final.UseNoise = 0;
    final.NoiseSeed = 0;
    final.NoiseOctaves = 0;
    final.NoiseFrequency = 0;
    final.NoiseAmplitude = 0;
    final.NoiseTimeScale = 0;
    ...
    if (material.MaterialType != MATERIAL_TYPE_NONE)
    {
        ...
        {
            minDist = sdf_with_noise(p, data, material);
            final = material;
        }
    }
    ...
    for (int i = 1; i < _SDFDataCount; i++)
    {
        ...
        if (material.MaterialType != MATERIAL_TYPE_NONE)
        {
            ...
            materialResult.MaterialType = 1;
            materialResult.TextureIndex = 0;
            materialResult.Colour = float3(0, 0, 0);
            materialResult.Emission = float3(0, 0, 0);
            materialResult.Metallic = 0;
            materialResult.Smoothness = 0;
            materialResult.Thickness = 0;
            materialResult.SubsurfaceColour = float3(0, 0, 0);
            materialResult.SubsurfaceScatteringPower = 0;
            materialResult.MaterialSmoothing = 0;
            materialResult.UsePackedUVs = 0;
            
            // Initialize noise fields
            materialResult.UseNoise = 0;
            materialResult.NoiseSeed = 0;
            materialResult.NoiseOctaves = 0;
            materialResult.NoiseFrequency = 0;
            materialResult.NoiseAmplitude = 0;
            materialResult.NoiseTimeScale = 0;
            ...
            {
                if (data.Operation == 0)
                    minDist = sdf_op_smin_material(minDist, sdf_with_noise(p, data, material), final, material, data.Smoothing, material.MaterialSmoothing, materialResult);
                else if (data.Operation == 1)
                    minDist = sdf_op_smoothSubtraction_material(minDist, sdf_with_noise(p, data, material), final, material, data.Smoothing, material.MaterialSmoothing, materialResult);
                else
                    minDist = sdf_op_smoothIntersection_material(minDist, sdf_with_noise(p, data, material), final, material, data.Smoothing, material.MaterialSmoothing, materialResult);
    ...
    // Ensure final is always properly initialized (suppress compiler warning)
    return final;
}
...
float2 MapUV(float3 p)
{
    ...
    for (int i = 0; i < _SDFDataCount; i++)
    {
        ...
        {
            SDFMaterialGPU material = _SDFMaterials[i];
            inverseDistanceSum += (1.0 / clamp(sdf_with_noise(tempP, _SDFData[i], material), smallNumber, bigNumber));
        }
    }
