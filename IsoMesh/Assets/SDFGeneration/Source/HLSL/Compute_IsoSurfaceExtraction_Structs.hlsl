...
SDFMaterialGPU lerpMaterial(SDFMaterialGPU a, SDFMaterialGPU b, float t)
{
    ...
    result.UsePackedUVs = a.UsePackedUVs; // Boolean field - use first material's value
    
    // Noise fields - interpolate continuous values, use first material for discrete values
    result.UseNoise = a.UseNoise; // Boolean - use first material
    result.NoiseSeed = a.NoiseSeed; // Seed should be consistent - use first material
    result.NoiseOctaves = a.NoiseOctaves; // Discrete - use first material
    result.NoiseFrequency = lerp(a.NoiseFrequency, b.NoiseFrequency, t); // Interpolate
    result.NoiseAmplitude = lerp(a.NoiseAmplitude, b.NoiseAmplitude, t); // Interpolate
    result.NoiseTimeScale = lerp(a.NoiseTimeScale, b.NoiseTimeScale, t); // Interpolate
...
SDFMaterialGPU negate(SDFMaterialGPU mat)
{
    ...
    result.UsePackedUVs = mat.UsePackedUVs; // Boolean field - keep as is
    
    // Noise fields
    result.UseNoise = mat.UseNoise; // Boolean - keep as is
    result.NoiseSeed = mat.NoiseSeed; // Seed - keep as is
    result.NoiseOctaves = mat.NoiseOctaves; // Discrete - keep as is
    result.NoiseFrequency = -mat.NoiseFrequency; // Negate continuous values
    result.NoiseAmplitude = -mat.NoiseAmplitude; // Negate continuous values
    result.NoiseTimeScale = -mat.NoiseTimeScale; // Negate continuous values
